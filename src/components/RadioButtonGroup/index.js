...
const RadioButtonGroup = ({
    ...
    const clones = validChildren.map((child, index) => {
      ...
      const spacingClasses = isInline ? `mr-${spacing}` : `mb-${spacing}`;
      ...
      return cloneElement(child, {
        ...
        isChecked,
        className: `${!isLastChild ? spacingClasses : ""} ${child.props.className}`
      });
    });
  
    return (
      <div role="radiogroup" onKeyDown={handleKeyDown} {...rest}>
        {clones}
      </div>
    );
  };
